        ...
        const uint32_t& max_msg_size_no_frag)
{
    ...
    auto ioServiceFunction = [&]()
            {
                ...
                asio::executor_work_guard<asio::io_context::executor_type> work(io_service_.get_executor());
#else
    ...
    if (0 < configuration()->keep_alive_frequency_ms)
    {
        auto ioServiceTimersFunction = [&]()
                {
#if ASIO_VERSION >= 101200
                    asio::executor_work_guard<asio::io_context::executor_type> work(io_service_timers_.
                                    get_executor());
#else
